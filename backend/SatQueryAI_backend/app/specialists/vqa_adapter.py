...
from app.models.placeholders import PlaceholderVQAModel
from app.schemas.image import ImageContext
from app.specialists.base import BaseSpecialistAdapter


class VisualVQAAdapter(BaseSpecialistAdapter):
    """Specialist adapter for Visual Question Answering (VQA) on satellite imagery."""

    def __init__(self, model: Optional[BaseSpecialistModel] = None):
        super().__init__(name="VQA", model=model or PlaceholderVQAModel())

    def prepare_inputs(self, query: str, images: List[ImageContext]) -> Dict[str, Any]:
        """
        Prepares inputs for VQA model.
        Requires at least one ImageContext object.
        """
        if not images:
            raise ValidationException(
                message="Visual VQA specialist requires at least one satellite image.",
